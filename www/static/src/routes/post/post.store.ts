...
class PostStore {
            ...
            .pipe(
                map((res) => {
                    res.data.data.map((post, i) => {
                        post.key = post.id;
                        post.author = post.user.name;
                        post.statusText = tools.getStatusText(
                            post.status,
                            post.create_time,
                        );
                    });
    ...
    passPostById(id: number) {
        http.post<any>(`/admin/api/post/${id}?method=put`, { status: 3 }).subscribe(
            (res) => {
                if (res.errno === 0) {
    ...
    refusePostById(id: number) {
        http.post<any>(`/admin/api/post/${id}?method=put`, { status: 2 }).subscribe(
            (res) => {
                if (res.errno === 0) {
