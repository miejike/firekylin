module.exports = class extends (
    ...
    getLastPostListByCate() {
        return think.cache('lastPostListByCate', async () => {
            const cateList = await this.model('cate').select();
            return await Promise.all(
                cateList.map(async ({ id, name, pathname }) => {
                    const field = [
                        `${this.config.prefix}post.id`,
                        'title',
                        'user_id',
                        'create_time',
                        'update_time',
    ...
    async getPostList(page, options = {}) {
        page = page | 0 || 1;
        const postsListSize = options.size || (await this.getPostsListSize());

        let field =
            options.field || 'id,title,pathname,create_time,summary,comment_num,options';
     ...
     * 根据cate获取第一条数据
     */
    getFirstPostByCate(id) {
        let where = this.getWhereCondition({ 'cate.cate_id': id });
        return this.join({
            table: 'post_cate',
            as: 'cate',
            on: ['id', 'post_id'],
     ...
     */
    getPostById(id) {
        let where = this.getWhereCondition({ id });
    ...
    async getPostDetail(pathname) {
        ...
        let detail = await this.where(where)
            .fieldReverse('markdown_content,summary')
            .find();
